...
<div  ng-controller="loginCtr">
	...
	<div class="f-prz formWrapLR">
		<form class="loginReg-form"  id="login-form" method="POST" action="{{ route('login') }}">
		{{ csrf_field() }}
			...
			<div class="filed">
				<input  type="password" name="password" placeholder="Password" id="pwd"  class="ipt pwd" datatype="*"  nullmsg="{{'This field is required'}}"/>
			</div>
			
			<div class="filed">
				
				<input  type="submit" value="Login" id="login-btn"  class="btn btn-orange btn-block" />
			</div>
			
			<div class="filed">
				<p class="login-tip f-cb"><span class="f-fr" ng-if="notChinese">Not on Binance yet? <a href="{{ route('register') }}" class="yellow">Register</a></span><a href="{{ route('password.request') }}" class="yellow f-fl">Forgot Password</a></p>
			</div>
			...
			<div class="verifyBottom">
				<div class="item lang">
					<a href="/">Home</a>　<a ng-href="/">Support</a>　|　<span ng-click="switching('en')" ng-class="{true:'black',false:''}[cur_lang=='en']">English</span> &nbsp;&nbsp;<span ng-click="switching('cn')" ng-class="{true:'black',false:''}[cur_lang=='cn']"></span>
				</div>
