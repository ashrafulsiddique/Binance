                    ...
                    <div class="col-12">
                        ...
                        <table class="table table-hover table-bordered" style="position: relative;margin-top:100px;top: -100px;">
                            ...
                            <tbody>
                                ...
                                @foreach ($Trade as  $open )
                                        <tr>
                                            <td><a href="{{ route('trade',str_replace('/','_',$open['date']) ) }}">{{ $open['date']  }}</a></td>
                                            <td><a href="{{ route('trade',str_replace('/','_',$open['pair']) ) }}">{{ $open['pair']  }}</a></td>
                                            <td><a href="{{ route('trade',str_replace('/','_',$open['type']) ) }}">{{ $open['type']  }}</a></td>
                                            <td><a href="{{ route('trade',str_replace('/','_',$open['price']) ) }}">{{ $open['price']  }}</a></td>
                                            <td><a href="{{ route('trade',str_replace('/','_',$open['filled']) ) }}">{{ $open['filled']  }}</a> </td>
                                            <td><a href="{{ route('trade',str_replace('/','_',$open['fee']) ) }}">{{ $open['fee']  }}</a></td>
                                            <td><a href="{{ route('trade',str_replace('/','_',$open['total']) ) }}">{{ $open['total']  }}</a></td>
